	...
	void Set (bool state)
	{
		...
		else if (mChecked != state)
		{
			...
			if (checkSprite != null)
			{
				...
				c.a = mChecked ? 1f : 0f;
				checkSprite.enabled = false; // eyal - for some odd reason we need to disable and enable this component in order for it to work
				checkSprite.enabled = true;  // strange , but seems to do the work
